...
void print_options(struct input_struct input, unsigned long* lengths, int generalizer_count, char** generalizers) {
    char empty_printed = 0;
...
int process(struct input_struct input, int generalizer_count, char** generalizers) {
    ...
    if (matched_len == 0) {
        print_options(input, lengths, generalizer_count, generalizers);
        free(lengths);
        free(match_indices);
        return 0;
    }
    ...
    if (nonempty) {
        process(input, generalizer_count, generalizers);
    }
    for (unsigned long i = 0; i < input.count; i++) {
...
int main (int input_count, char** inputs) {
    ...
    int generalizer_count = 0;
    char** generalizers = malloc((input_count / 2) * sizeof(char));
    ...
    while ((opt = getopt(input_count, inputs, "g:d:")) != -1) {
        ...
        case 'g':
            arg_count += 2;
            printf("Using generalizer regex mode %s\n", optarg);
            generalizers[generalizer_count] = optarg;
            generalizer_count++;
            break;
        case 'd':
            arg_count += 2;
